...
版本：V1.0
日期：2026-05-04
关联文档：`后端开发规范-V1.0.md`、`数据库设计详细文档-V1.0.md`
Base URL：`/api/v1`
...
- 后台接口需要 `Authorization: Bearer <token>` 请求头
- 前台接口和监控上报接口无需鉴权
- Token 过期返回 `40100`
...
说明：专题数据复用 `category` 表，按 "专题" 的视角对外展示（与 2.4 分类列表共用同一数据源，但响应字段侧重专题展示）。

响应类似分类列表，增加专题封面图和聚合文章数。

## 2.8 获取专题详情

```
GET /api/v1/public/topics/{topic_id}
```
响应包含专题信息和关联的文章列表（分页）。
...
```
无需请求体。无鉴权，基于 IP 去重（Redis 缓存 30 分钟）。
...
| category_id | status=published 时必选 |
| tag_ids | 可选，最多 10 个 |
