...
```
POST /api/v1/public/search/semantic
```
...
| 参数 | 类型 | 必填 | 默认值 | 说明 |
|------|------|------|--------|------|
| q | string | 是 | - | 搜索查询（1-500字符），支持自然语言描述 |
| page | int | 否 | 1 | 页码（≥1） |
| page_size | int | 否 | 20 | 每页数量（1-50） |
| threshold | float | 否 | 0.3 | 相似度阈值（0.0-1.0），越高匹配越精确 |

搜索原理：将查询文本通过 BGE Embedding 模型向量化，使用 pgvector 余弦相似度检索 `vector_chunk` 表，按文章去重后以相关度降序返回。与关键词搜索互补：语义搜索按含义匹配，无需精确关键词；关键词搜索按字符匹配，结果更可控。

响应：分页文章列表（同文章列表接口格式），每个结果额外包含：
- `relevance`：相关度分数（0-1，float）
- `snippet`：匹配片段（前200字符）

错误码：
- `422`：查询参数非法（空查询、超长、阈值越界）
- `503`：Embedding 模型未加载
