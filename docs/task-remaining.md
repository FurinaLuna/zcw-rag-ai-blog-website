...
当前状态: 14 个前端页面 + 18 个组件 + 49 条 API 路由 + 66 个后端测试 + 15 个前端测试 + 6 个 E2E 测试

---

## 剩余任务 (按优先级排列)

### 任务 1: RAG 流式输出 (SSE) 端点

**优先级**: P0 -- 面试演示的核心差异化功能

**当前状态**: `/api/v1/rag/ask` 仅支持非流式回答，用户需等待完整响应。前端 `pages/ask.vue` 和 `RAGFloatingButton` 均为一次性请求-响应模式。

**缺失内容**:
- 后端 `POST /api/v1/rag/ask/stream` SSE 端点 (任务清单 2.3 列为 P1)
- 后端 `app/services/rag/generator.py` 的 `generate_answer_stream` 异步生成器
- 前端 SSE EventSource 消费逻辑 (逐 token 渲染到消息气泡)
- LLM API 调用需传递 `stream: true` 参数

**验收标准**:
- `POST /api/v1/rag/ask/stream` 返回 `text/event-stream`
- 前端 ask 页和浮动按钮逐步显示回答内容
- 流中断时（网络断开/服务端错误）前端显示友好提示并支持重试
- sources 引用在流结束后追加显示
